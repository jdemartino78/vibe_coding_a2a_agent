...
from .local_base_agent import LocalBaseAgent
from a2a_multiagent_mcp_app.a2a_agents.common.remote_connection import TaskCallbackArg
import base64
import datetime
import json
import os
...
class ADKHostManager(ApplicationManager):
    ...
    def __init__(
        ...
        # self._session_service = InMemorySessionService() # This will be replaced
        self._artifact_service = InMemoryArtifactService()
        self._memory_service = InMemoryMemoryService()

        # New code to use VertexAiSessionService
        project_id = os.environ.get("PROJECT_ID")
        location = os.environ.get("LOCATION")
        agent_engine_id = os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID")

        if not project_id or not location or not agent_engine_id:
            raise ValueError(
                "Environment variables PROJECT_ID, LOCATION, and GOOGLE_CLOUD_AGENT_ENGINE_ID must be set."
            )
        ...
        self._host_agent = None # No longer directly instantiating AdkOrchestratorAgent
        ...
        app = App(
            name=self.app_name,
            root_agent=LocalBaseAgent(name="dummy_agent"),
        )
        ...
        c = StateConversation(conversation_id="dummy_conversation", is_active=True)
    ...
    def emit_event(self, task: TaskCallbackArg):
        content = None
